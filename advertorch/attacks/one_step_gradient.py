...
class GradientSignAttack(Attack, LabelMixin):
    ...
    def __init__(self, predict, loss_fn=None, eps=0.3, clip_min=0.,
                 clip_max=1., targeted=False,eot_samples=1):
        """
    ...
    def perturb(self, x, y=None):
        ...
        avg_grad = torch.FloatTensor(x.shape)
        avg_grad.zero_()
        if x.is_cuda:
        ...
        for _ in range(self.eot_samples):
            ...
            if self.targeted:
                loss = -loss
            loss.backward()
            avg_grad += xadv.grad.detach()
        avg_grad = avg_grad/self.eot_samples
    
        grad_sign = avg_grad.detach().sign()
...
class GradientAttack(Attack, LabelMixin):
    ...
    def perturb(self, x, y=None):
        ...
        grad = normalize_by_pnorm(avg_grad.detach())
